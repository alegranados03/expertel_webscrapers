...
class TelusMonthlyReportsScraperStrategy(MonthlyReportsScraperStrategy):
    ...
    def _download_files(
        ...
        """Descarga los archivos mensuales de Telus."""
        downloaded_files = []
        ...
        try:
            ...
            file_info = FileDownloadInfo(
                file_id=1,
                file_name=f"telus_monthly_report_{billing_cycle.start_date}_{billing_cycle.end_date}.pdf",
                download_url="https://telus.com/download/monthly_report.pdf",
                file_path=f"/tmp/telus_monthly_{billing_cycle.id}.pdf",
                file_size=1024000,
            )
...
class TelusDailyUsageScraperStrategy(DailyUsageScraperStrategy):
    ...
    def _download_files(
        ...
        """Descarga los archivos de uso diario de Telus."""
        downloaded_files = []
        ...
        try:
            ...
            file_info = FileDownloadInfo(
                file_id=1,
                file_name=f"telus_daily_usage_{billing_cycle.start_date}_{billing_cycle.end_date}.csv",
                download_url="https://telus.com/download/daily_usage.csv",
                file_path=f"/tmp/telus_daily_{billing_cycle.id}.csv",
                file_size=512000,
            )
            downloaded_files.append(file_info)
...
class TelusPDFInvoiceScraperStrategy(PDFInvoiceScraperStrategy):
    ...
    def _download_files(
        ...
        try:
            ...
            file_info = FileDownloadInfo(
                file_id=1,
                file_name=f"telus_invoice_{billing_cycle.start_date}_{billing_cycle.end_date}.pdf",
                download_url="https://telus.com/download/invoice.pdf",
                file_path=f"/tmp/telus_invoice_{billing_cycle.id}.pdf",
                file_size=2048000,
            )
            downloaded_files.append(file_info)
