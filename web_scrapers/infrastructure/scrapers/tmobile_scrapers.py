...
class TMobileDailyUsageScraperStrategy(DailyUsageScraperStrategy):
    ...
    def _download_files(
        ...
        try:
            ...
            file_info = FileDownloadInfo(
                file_id=1,
                file_name=f"tmobile_daily_usage_{billing_cycle.start_date}_{billing_cycle.end_date}.csv",
                download_url="https://tmobile.com/download/daily_usage.csv",
                file_path=f"/tmp/tmobile_daily_{billing_cycle.id}.csv",
                file_size=512000,
            )
            downloaded_files.append(file_info)
...
class TMobilePDFInvoiceScraperStrategy(PDFInvoiceScraperStrategy):
    """Scraper de facturas PDF para T-Mobile."""
    ...
    def _download_files(
        ...
        """Descarga las facturas PDF de T-Mobile."""
        downloaded_files = []
        ...
        try:
            ...
            file_info = FileDownloadInfo(
                file_id=1,
                file_name=f"tmobile_invoice_{billing_cycle.start_date}_{billing_cycle.end_date}.pdf",
                download_url="https://tmobile.com/download/invoice.pdf",
                file_path=f"/tmp/tmobile_invoice_{billing_cycle.id}.pdf",
                file_size=2048000,
            )
            downloaded_files.append(file_info)
